...
![The figure from @laube2011 visualizes steps a) zu d), which will be explained below](../02_Images/laube_2011.jpg){}
...
```{r}
#| echo: false
#| fig.cap: paste0("Movement of the wildboar 'Sabi' in the timespan ", strftime(min(sabi$DatetimeUTC), "%d"), "-",strftime(max(sabi$DatetimeUTC), "%d.%m.%Y"), ". The circle highlingts possible 'static points'")
...
With the skills from the input above you can now implement the segmentation algorithm described in @laube2011 on the dataset `r download_url("caro60.csv","datasets")`. Download this dataset (right click > save target as...) and import it as a simple `data.frame` or `tibble` (you don't need an `sf`-object for today's task).
